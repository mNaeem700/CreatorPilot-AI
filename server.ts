...
// Enable CORS with support for credentials, custom headers (x-user-id, Authorization)
app.use(cors({
  origin: (origin, callback) => {
    // Dynamically echo back any requesting origin to satisfy the 'credentials: true' requirement
...
async function queryGemini(prompt: string, fallbackText: string, sysPromptOverride?: string): Promise<string> {
  ...
  const primaryModel = db.adminSettings.activeModel || "gemini-3.5-flash";
  const fallbackModels = ["gemini-3.5-flash", "gemini-3.1-flash-lite", "gemini-3.1-pro-preview"].filter(m => m !== primaryModel);
  const modelQueue = [primaryModel, ...fallbackModels];

  const sysInstruction = sysPromptOverride || db.adminSettings.systemPrompt;
  let lastError: any = null;

  for (const selectedModel of modelQueue) {
    try {
      console.log(`[Gemini Request] Attempting query with model: ${selectedModel}`);
      ...
      console.error(`Gemini invocation failed for model ${selectedModel}:`, err);
      logAction("warn", `Gemini invocation warning on model ${selectedModel}: ${err.message || err}`);
      lastError = err;
    }
  }

  logAction("error", `All Gemini models in queue failed. Last error: ${lastError?.message || lastError}`);
...
async function queryGeminiMultimodal(prompt: string, base64Image: string, mimeType: string, fallbackText: string): Promise<string> {
  ...
  const modelQueue = [primaryModel, "gemini-3.1-flash-lite", "gemini-3.1-pro-preview"];
  let lastError: any = null;
  ...
  for (const selectedModel of modelQueue) {
    try {
      console.log(`[Gemini Request Multimodal] Attempting query with model: ${selectedModel}`);
      const response = await ai.models.generateContent({
        model: selectedModel,
        contents: {
      ...
      console.error(`Gemini Multimodal invocation failed for model ${selectedModel}:`, err);
      logAction("warn", `Gemini Multimodal warning on model ${selectedModel}: ${err.message || err}`);
      lastError = err;
    }
  }

  logAction("error", `All Gemini Multimodal models in queue failed. Last error: ${lastError?.message || lastError}`);
...
app.get("/api/admin/settings", checkAdmin, (req, res) => {
  res.json({
    settings: {
      ...db.adminSettings,
      defaultModel: db.adminSettings.activeModel || "gemini-3.5-flash",
